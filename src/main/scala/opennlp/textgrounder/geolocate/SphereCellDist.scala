 ...
 * file showing the cell distribution of a given gram across the Earth);
 * the `average-cell-probability` ranker (which uses the inversion strategy
 * to obtain a distribution of cells for each gram in a document, and then
...
object SphereGramCellDist {
  ...
  def generate_kml_file(celldist: GramCellDist[SphereCoord],
    ...
    // To convert a gram to a string without assuming that we necessarily
