...
"""
  ...
  def run_program(args: Array[String]) = {
    ...
    if (params.location_histogram != null) {
      ...
      val runs =
        for ((label, fn) <- run_fns) yield (label, stats.map(fn))
      compute_histogram(params.location_histogram, runs, 3, 2)
    }

    if (params.results_histogram != null) {
      val base = LocationVarianceParams.bin_base
      val max_bin = LocationVarianceParams.num_bins - 1
      for ((file_label, fn) <- run_fns) {
        errprint("\nFile label: %s", file_label)
        val binned_merged_stats = merged_stats.groupBy {
          case (stat, result) =>
            val bin_by = fn(stat)
        ...
        val runs = binned_merged_stats.map {
          case (bin, results) => {
            ...
            errprint("Mean: %.2f km, Median: %.2f km", mean(run), median(run))
            (bin_label, run)
          }
        }.filter {
