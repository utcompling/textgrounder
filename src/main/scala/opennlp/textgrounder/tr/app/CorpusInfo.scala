...
object CorpusInfo {
  ...
  def getCorpusInfo(filename: String): collection.mutable.HashMap[String, collection.mutable.HashMap[Int, Int]] = {

    val tokenizer = new OpenNLPTokenizer

    val corpus = Corpus.createStoredCorpus
    corpus.addSource(new TrXMLDirSource(new File(filename), tokenizer))
  ...
  def main(args:Array[String]) = {
    printCorpusInfo(getCorpusInfo(args(0)))
  }
