...
public abstract class Annealer {
    ...
    protected void addToFirstMoment(double[] _target, double[] _source) {
        try {
            for (int i = 0;; ++i) {
                _target[i] = Math.exp(TGMath.stableLogSum(Math.log(_target[i]), Math.log(_source[i])));
            }
    ...
    protected void addToSecondMoment(double[] _target, double[] _source) {
        try {
            for (int i = 0;; ++i) {
                _target[i] = Math.exp(TGMath.stableLogSum(Math.log(_target[i]), 2 * Math.log(_source[i])));
            }
        } catch (ArrayIndexOutOfBoundsException e) {
        }
    }
    ...
    protected void addToCovariance(double[] _target, double[] _source) {
        try {
            ...
            for (int i = 0; i < c; ++i) {
                ...
                for (int j = i; j < c; ++j) {
                    _target[off + j] = Math.exp(TGMath.stableLogSum(Math.log(_target[i]), Math.log(_source[i]) + Math.log(_source[j])));
                }
            }
        } catch (ArrayIndexOutOfBoundsException e) {
        }
